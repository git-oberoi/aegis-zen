...
export const generateDailyBriefing = async ({ tasks, habits, history, selectedDate, apiKey }) => {
  const cacheKey = `briefing_${selectedDate}`;
  ...
  try {
    const prompt = buildBriefingPrompt(tasks, habits, history);
    ...
    const fallback = parseHeuristicBriefing(tasks, habits, history);
